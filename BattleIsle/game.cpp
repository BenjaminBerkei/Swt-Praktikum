...
Game::Game(Options *init_options, GameWidget *ptr_gameWid) :
    ptr_hexSelectionCache(nullptr), ptr_gameOptions(init_options), ptr_gameWidget(ptr_gameWid),
...
/*Starten eines Spiels mit den Optionen definiert in init_Options*/
    ...
    ptr_roundCurrent = new Round(ptr_gameOptions->getInt_roundLimit());

    if(!loadMapForNewGame(ptr_gameOptions->getStr_map()))
    {
        createRandomMap();
    }
...
bool Game::readSaveGame(QString filepath)
{
    ...
    ptr_gameOptions = Options::unserialize(in);

    if( ptr_gameOptions->getBool_ki() == true)
    {
    ...
    loadMapFromSaveGame(ptr_gameOptions->getStr_map());
